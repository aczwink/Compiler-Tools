...
//Private Functions
InputLexer::ECharClass InputLexer::GetCharClass(char c)
{
    switch(c)
    {
        case 0:
...
InputLexer::EState InputLexer::GetNextState(EState state, ECharClass cc)
{
    switch(state)
    {
        case STATE_START:
...
ESourceToken InputLexer::GetTokenType(EState state)
{
    switch(state)
    {
        case STATE_END:
...
//Public Functions
uint32 InputLexer::GetLineNumber() const
{
    return this->lineNumber;
}

ESourceToken InputLexer::GetNextToken()
{
    ...
    for(uint32 i = 0; i < uint32(this->pSource - pStart); i++)
    {
        this->tokenValue += pStart[i];
    }
...
const String &InputLexer::GetTokenValue()
{
    return this->tokenValue;
}

void InputLexer::Init(const char *pSource)
{
